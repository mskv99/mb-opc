...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Enter path to folder with gds files and layer number to create an image"
    )
    parser.add_argument(
        "input_folder_path", type=str, help="Path for input folder with gdsii files"
    )
    parser.add_argument(
        "output_folder_path", type=str, help="Path for output folder with images"
    )
    parser.add_argument(
        "layer_number",
        type=int,
        help="Number of layer in topology file; 3 - refers to original, 100 - refers to train_correction",
    )
