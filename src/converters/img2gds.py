...
cell.add(gdspy.Polygon([(0,0), (0,1), (1,1), (1,0)], layer = 5)) # добавили референсный полигон
...
for i,poly_position in enumerate(list(zip(white_pix_indices[1], 1024 - white_pix_indices[0]))):
...
for i in range(len(top_cell_polygons)):
  if i == 0:
    union_figure = gdspy.boolean(top_cell_polygons[i], top_cell_polygons[i+1], operation = 'or', layer = 3)
  else:
    union_figure = gdspy.boolean(union_figure, top_cell_polygons[i], operation = 'or', layer = 3)
