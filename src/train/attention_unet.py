...
def validate_model(model, val_loader, current_epoch, num_epochs, checkpoint_dir, device='cuda'):
  ...
  return log_info_epoch['total_loss'] / log_info_epoch['len_valid_loader'], log_info_epoch['pixel_acc'] / \
         log_info_epoch['len_valid_loader'], log_info_epoch['iou'] / log_info_epoch['len_valid_loader']
...
def train_model(model, train_loader,
  ...
  if resume:
    checkpoint = torch.load(
      '/mnt/data/amoskovtsev/mb_opc/checkpoints/exp_3/last_checkpoint.pth')  # torch.load(os.path.join(checkpoint_dir, "checkpoint_14.10.24.pth"))
