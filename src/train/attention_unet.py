...
from models.components.attention_unet import AttentionUNetGenerator
...
def validate_model(
    ...
    with torch.no_grad():
        for idx, (image, target) in enumerate(val_loader):
            ...
            if idx % 10 == 0:
                ...
                log_message_iter = (
                    f"Epoch [{log_info_iter['epoch']}/{log_info_iter['num_epochs']}], "
        ...
        log_message_epoch = (
            f"Losses per epoch [{log_info_epoch['epoch']}/{log_info_epoch['num_epochs']}], "
    ...
    return (
        log_info_epoch["total_loss"] / log_info_epoch["len_valid_loader"],
        log_info_epoch["pixel_acc"] / log_info_epoch["len_valid_loader"],
        log_info_epoch["iou"] / log_info_epoch["len_valid_loader"],
    )
...
def train_model(
    ...
    if resume:
        checkpoint = torch.load(
            "/mnt/data/amoskovtsev/mb_opc/checkpoints/exp_3/last_checkpoint.pth"
        ...
        scheduler = lr_sched.CosineAnnealingLR(
            optimizer=optimizer, T_max=50, eta_min=1e-7
        )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for idx, (image, target) in enumerate(progress):
            ...
            if idx % 50 == 0:
                ...
                draw_plot(
                    first_variable=total_loss_iter_list,
        ...
        total_loss_epoch_list_train.append(
            log_info_epoch["total_loss"] / log_info_epoch["len_train_loader"]
        )
        pixel_acc_epoch_list_train.append(
            log_info_epoch["pixel_acc"] / log_info_epoch["len_train_loader"]
        )
        iou_epoch_list_train.append(
            log_info_epoch["iou"] / log_info_epoch["len_train_loader"]
        )

        # get average loss, pixel accuracy and iou per epoch during validation phase
        total_loss_epoch_val, pixel_acc_epoch_val, iou_epoch_val = validate_model(
            model,
            val_loader,
            current_epoch=epoch,
            num_epochs=num_epochs,
        ...
        draw_plot(
            first_variable=total_loss_epoch_list_train,
            second_variable=total_loss_epoch_list_val,
            title="Loss plot",
            xlabel="epoch",
            ylabel="loss",
