...
from models.components.damo import Generator, Discriminator
...
def validate_model(
    gen_model, val_loader, current_epoch, num_epochs, checkpoint_dir, device="cuda"
):
    gen_model.eval()
...
def train_model(
    ...
    schedG = lr_sched.CosineAnnealingLR(
        optimizer=optimG, T_max=num_epochs, eta_min=1e-7
    )
    schedD = lr_sched.CosineAnnealingLR(
        optimizer=optimD, T_max=num_epochs, eta_min=1e-7
    )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for idx, (image, target) in enumerate(progress):
            ...
            zeros = torch.zeros(
                [maskFake.shape[0]], dtype=maskFake.dtype, device=maskFake.device
            )
            maskTrue = target
            ones = torch.ones(
                [maskTrue.shape[0]], dtype=maskTrue.dtype, device=maskTrue.device
            )
            ...
            if idx % 50 == 0:
                ...
                draw_plot(
                    ...
                    second_variable=lossD_iter_list,
                    title="G and D iteration loss plot",
                    xlabel="iteration",
                    ylabel="loss",
                    first_label="generator",
                    second_label="discriminator",
            ...
            if idx % 500 == 0:
                generator_checkpoint_path = os.path.join(
                    checkpoint_dir, "generator_checkpoint.pth"
                )
                torch.save(
                    {
                        "epoch": epoch,
                ...
                logging.info(
                    f"Saved generator checkpoint at {generator_checkpoint_path}"
                )

                discriminator_checkpoint_path = os.path.join(
                    checkpoint_dir, "discriminator_checkpoint.pth"
                )
                torch.save(
                    {
                        "epoch": epoch,
                ...
                logging.info(
                    f"Saved generator checkpoint at {discriminator_checkpoint_path}"
                )
        ...
        draw_plot(
            ...
            second_variable=lossD_epoch_list_train,
            title="Epoch Loss Plot for train phase",
            xlabel="epoch",
            ylabel="loss",
            first_label="generator",
            second_label="discriminator",
            save_name="train_epoch_loss.jpg",
            checkpoint_dir=checkpoint_dir,
        )

        # draw epoch Generator loss for validation phase
        draw_plot(
            first_variable=lossG_epoch_list_val,
            label="generator",
            title="Epoch Loss plot for valid phase",
            xlabel="Iteration",
            ylabel="Loss",
            save_name="generator_val_epoch_loss.jpg",
            checkpoint_dir=checkpoint_dir,
        )
        ...
        draw_plot(
            ...
            title="Epoch Pixel Accuracy plot",
            xlabel="epoch",
