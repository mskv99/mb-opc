...
def sepconv2d(
    chIn, chOut, kernel_size, stride, padding, bias=True, norm=True, relu=False
):
    layers = []
    layers.append(
        nn.Conv2d(
...
def repeat2d(
    n, chIn, chOut, kernel_size, stride, padding, bias=True, norm=True, relu=False
):
    layers = []
    for idx in range(n):
        layers.append(
            nn.Conv2d(
                chIn if idx == 0 else chOut,
                chOut,
                kernel_size=kernel_size,
                stride=stride,
                padding=padding,
...
def spsr(
    r, chIn, chOut, kernel_size, stride, padding, bias=True, norm=True, relu=False
):
    layers = []
    layers.append(
        nn.Conv2d(
...
def apply_complex(fr, fi, input):
    return torch.complex(
        fr(input.real) - fi(input.imag), fr(input.imag) + fi(input.real)
    )
...
class CFNONet(nn.Module):
    def __init__(self):
        ...
        self.conv2b = repeat2d(
            2, 128, 128, kernel_size=3, stride=1, padding=1, relu=True
        )
        self.branch = nn.Sequential(
            self.conv0a, self.conv0b, self.conv1a, self.conv1b, self.conv2a, self.conv2b
        )

        self.deconv0a = deconv2d(
            16 + 32 + 64 + 128,
            128,
            kernel_size=3,
            stride=2,
        ...
        self.deconv0b = repeat2d(
            2, 128, 128, kernel_size=3, stride=1, padding=1, relu=True
        )
        self.deconv1a = deconv2d(
            128, 64, kernel_size=3, stride=2, padding=1, output_padding=1, relu=True
        )
        self.deconv1b = repeat2d(
            2, 64, 64, kernel_size=3, stride=1, padding=1, relu=True
        )
        self.deconv2a = deconv2d(
            64, 32, kernel_size=3, stride=2, padding=1, output_padding=1, relu=True
        )
        self.deconv2b = repeat2d(
            2, 32, 32, kernel_size=3, stride=1, padding=1, relu=True
        )
        ...
        self.conv6 = conv2d(
            32, 1, kernel_size=3, stride=1, padding=1, norm=False, relu=False
        )
